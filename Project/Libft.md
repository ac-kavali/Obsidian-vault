 ...
 **Casting** in C is explicitly converting a value from one data type to another.
 \- <span style="color:rgb(0, 176, 80)">implicit caste</span> → done _automatically_ by the compiler 
 (e.g., `int x = 5; float y = x;`)
 \- **<span style="color:rgb(0, 176, 80)">explicite caste</span>** → done _manually_ by the programmer using parentheses (e.g., `float y = (float)x;`)
...
based on your our systems let's carrefully know what is "*big and little endiannes".*
